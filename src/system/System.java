...
public class System {
    ...
    public void work() {
        ...
        double sumResp = 0, sumExec = 0;
        int num = 0;

        for (Process proc : finishedProcess) {
            sumResp+=proc.stats().getResponseTime();
            sumExec+=proc.stats().getExecutionTime();
            num++;
            proc.writeResults();
        }
        java.lang.System.out.println("Average response time: "+sumResp/num);
        java.lang.System.out.println("Average execution time: "+sumExec/num);
    }
